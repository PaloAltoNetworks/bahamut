	...
	Convey("Given I create a http.ResponseWriter", t, func() {
		...
		Convey("When I use setCommonHeader with a referer", func() {
			...
			Convey("Then the common headers should be set", func() {
				...
				So(w.Header().Get("Access-Control-Allow-Headers"), ShouldEqual, "Authorization, Content-Type, Cache-Control, If-Modified-Since, X-Requested-With, X-Count-Total, X-Namespace, X-External-Tracking-Type, X-External-Tracking-ID, X-TLS-Client-Certificate")
