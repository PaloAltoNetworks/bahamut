...
// FakeRetrieveManyProcessor
type FakeRetrieveManyProcessor struct {
	err error
}

func (p *FakeRetrieveManyProcessor) ProcessRetrieveMany(*Context) error {
	return p.err
}
...
func TestDispatchers_dispatchRetrieveManyOperation(t *testing.T) {
	...
	Convey("Given I have an authenticator that is not authenticated", t, func() {
		request := elemental.NewRequest()
	...
	Convey("Given I have an authorizer that is not authorize", t, func() {
		request := elemental.NewRequest()
